  ...
  return (
    <section className="rounded-3xl border border-white/80 bg-white/85 p-6 shadow-xl shadow-indigo-100/60">
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
        <h2 className="text-xl font-bold text-slate-900">Interviewer Question</h2>
        <p className="mt-2 leading-7 text-slate-700">{question.question_text}</p>
      </div>

      <textarea
        className="mt-5 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 outline-none ring-sky-400/70 transition placeholder:text-slate-400 focus:ring-2"
        value={answer}
        onChange={(e) => handleAnswerChange(e.target.value)}
        placeholder="Write your answer here. Structure: context, approach, implementation details, trade-offs."
        rows={7}
      />
      <div className="mt-2 flex items-center justify-between text-xs text-slate-500">
        <span>Tip: keep answers clear and technical, with short examples.</span>
      ...
      {lastAnswerFeedback && (
        <div className="mt-5 rounded-2xl border border-indigo-100 bg-indigo-50/70 p-4">
          <p className="text-sm text-slate-700">
            <b>Evaluator note - score:</b> {lastAnswerFeedback.score}
          </p>
